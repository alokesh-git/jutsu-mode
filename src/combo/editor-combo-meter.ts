...
export class EditorComboMeter implements Plugin<EditorComboMeterConfig> {
    ...
    private isjutsupowerActive: boolean = false;
    private initialjutsupowerCombo: number = 0;
    ...
    public onjutsupowerStart = (combo: number) => {
        this.isjutsupowerActive = true;
        this.initialjutsupowerCombo = combo;
    }

    public onjutsupowerStop = (finalCombo: number) => {
        this.isjutsupowerActive = false;
        this.initialjutsupowerCombo = 0;
    }
    ...
    public onDidChangeTextDocument = (data: jutsupowerChangeTextDocumentEventData, event: vscode.TextDocumentChangeEvent) => {
        ...
        this.isjutsupowerActive = data.isjutsupowerActive;
    ...
    private getSharedStyles = (comboCount: number, frameCount = 0): { textSize: string, color: string } => {
        // Because the size and color do not start to change until jutsu power starts, we cannot use the raw "count" to calculate those values
        // or else there will be a large jump when jutsupower activates, so instead use the value relative to the combo at which jutsu power started.
        const jutsupowerCombo = this.isjutsupowerActive ? comboCount - this.initialjutsupowerCombo : 0;

        const baseTextSize = this.config.comboCounterSize;
        const styleCount = Math.min(jutsupowerCombo, 25);
        // TODO: Explain how this formula works
        const textSize = this.isjutsupowerActive ? ((styleCount * baseTextSize) / 100 * Math.pow(0.5, frameCount * 0.2) + baseTextSize) : baseTextSize;
        // Only change color in jutsu power
        const color = `hsl(${(100 - (this.isjutsupowerActive ? jutsupowerCombo : 0) * 1.2)}, 100%, 45%)`;
    ...
    private createComboCountDecoration = (count: number, ranges: vscode.Range[], editor: vscode.TextEditor = vscode.window.activeTextEditor) => {
        ...
        const animateComboCountDecoration = (frameCount: number) => {
            ...
            // Only animate in jutsu power
            if (this.isjutsupowerActive && frameCount < 100) {
