...
import { TieJutsu } from './config/guysainsai';
import {FlashJutsu} from "./config/minato";
import {KakasiJutsu} from "./config/kakasi"
import { Magic } from './config/magic';
import {Kamabuntai} from "./config/kamabuntai"
...
let themes: {[key: string]: ThemeConfig} = {
    ...
    flames: Flames,
    tieJutsu:TieJutsu,
    flashJutsu: FlashJutsu,
    kakasiJutsu:KakasiJutsu,
    magic: Magic,
    kamabuntai:Kamabuntai,
    clippy: Clippy,
...
// Current combo count
let combo = 0;
let isjutsupowerActive = false;

export function activate(context: vscode.ExtensionContext) {
    // Try to migrate any existing configuration files
    migrateConfiguration();

    const enableCommand = 'jutsu-power.enablejutsupower';
    const disableCommand = 'jutsu-power.disablejutsupower';

    const setEnabled = (value: boolean) => {
        const config = vscode.workspace.getConfiguration("jutsu-power");
...
const onComboTimerExpired = () => {
    plugins.forEach(plugin => plugin.onjutsupowerStop(combo));
...
function onDidChangeTextDocument(event: vscode.TextDocumentChangeEvent) {
    ...
    combo++;
    const jutsupower = isjutsupower();

    startTimer();

    if (jutsupower != isjutsupowerActive) {
        isjutsupowerActive = jutsupower;

        isjutsupowerActive ?
            plugins.forEach(plugin => plugin.onjutsupowerStart(combo)) :
            plugins.forEach(plugin => plugin.onjutsupowerStop(combo));
    }
